...
export default function FinanceView({ transactions, setTransactions, categories }: FinanceViewProps) {
    ...
    const addTransaction = () => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            newTransactions.push({
                ...
                isCompleted: false, // Starts as pending/not completed
    ...
    // --- CÁLCULOS PARA RELATÓRIO SINCRONIZADO ---
    const { incomeTotal, incomeReceived, cashExpenseTotal, cashExpensePaid } = useMemo(() => {
        const iTotal = monthlyTransactions.filter(t => t.type === 'income').reduce((acc, t) => acc + Number(t.value || 0), 0);
        const iReceived = monthlyTransactions.filter(t => t.type === 'income' && t.isCompleted).reduce((acc, t) => acc + Number(t.value || 0), 0);
        
        const ceTotal = monthlyTransactions.filter(t => t.type === 'expense').reduce((acc, t) => acc + Number(t.value || 0), 0);
        const cePaid = monthlyTransactions.filter(t => t.type === 'expense' && t.isCompleted).reduce((acc, t) => acc + Number(t.value || 0), 0);
    ...
    const reportPendingExpense = cashExpenseTotal - cashExpensePaid;
    const reportRealizedBalance = incomeReceived - cashExpensePaid;
    const reportProjectedBalance = incomeTotal - cashExpenseTotal;
    ...
    }, [monthlyTransactions, filter, sortOrder, searchTerm]);
                                    ...
                                    <div className="pt-6 border-t-2 border-slate-950/5 dark:border-white/5 space-y-4">
                                        ...
                                        <div className="flex justify-between items-center p-4 rounded-3xl bg-primary/10 border border-primary/20 shadow-inner">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className={cn("text-xs font-black", reportPendingIncome - reportPendingExpense >= 0 ? "text-orange-500" : "text-destructive/80")}>
                                                    {formatCurrency(reportPendingIncome - reportPendingExpense)}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <div className="flex items-center gap-2 mt-1">
                                                            ...
                                                            <span className="text-[9px] sm:text-[10px] font-medium text-slate-500 dark:text-muted-foreground/40 italic truncate">
                                                                {t.paymentMethod === 'credit' ? 'Cartão de Crédito' : 'Saldo em conta'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-1 sm:gap-4 ml-2 shrink-0">
                                                        <div className="text-right">
                                                            ...
                                                            <div className="flex items-center justify-end gap-1 mt-1 opacity-60">
                                                                ...
                                                                {t.paymentMethod === 'credit' ? <Wallet className="h-2.5 w-2.5" /> : <Wallet className="h-2.5 w-2.5" />}
                                                            </div>
                                                        </div>
